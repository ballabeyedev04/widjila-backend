 ...
 *
 * Pas de capture locale : l'échec remonte à `envelopperJob`, qui le
 * journalise avec sa pile et le compte (voir utils/executerJob.js). L'ancien
 * `logger.error('…', err.message)` perdait le motif.
 */
...
// Deux exécutions enveloppées DISTINCTES : l'échec des relances ne doit pas
// empêcher l'escalade (et inversement), et chacune a son propre historique.
// Pas de rattrapage au démarrage : ces rappels sont attendus à la prise de
// poste, pas au milieu de la nuit après un redémarrage — l'anti-doublon
// (`dejaNotifie`) les rattrape de toute façon au passage suivant.
const executerRelances = envelopperJob('rappels-echeances', relancerEcheances);
const executerEscalade = envelopperJob('escalade-retards', escaladerRetards);
...
function startRemindersJob() {
  const task = cron.schedule('0 7 * * *', async () => {
    // Les enveloppes ne lèvent jamais : la seconde s'exécute quoi qu'il arrive.
    await executerRelances();
    await executerEscalade();
  }, { timezone: CRON_TZ });
